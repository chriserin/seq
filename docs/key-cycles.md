...
for this section are increased. Because sq has playback cursors for each line, one line must
...
next part. For Overlays, we use the Key Cycles to determine the Overlay to
...
There are various actions (SEE [ACTIONS](docs/actions.md)) that you can place
in any line to manipulate the playback cursor for that line. For instance, if
you add the line reset action (`ss`) to the line with the Key indicator at beat
5, then when playback occurs only the first 4 beats of that line will play
before the playback cursor for that line resets to zero, which will increase
the amount of Key Cycles.
