...
func CompiledAPI() (apis []API) {
	n := C.macmidi_get_compiled_api(nil, 0)
	capis := make([]C.enum_MacMidiApi, n, n)
	C.macmidi_get_compiled_api(&capis[0], C.uint(n))
...
func (m *midi) OpenPort(port int, name string) error {
	...
	C.macmidi_open_port(m.midi, C.uint(port), p)
	if !m.midi.ok {
...
func (m *midi) OpenVirtualPort(name string) error {
	p := C.CString(name)
	defer C.free(unsafe.Pointer(p))
	C.macmidi_open_virtual_port(m.midi, p)
	if !m.midi.ok {
		return errors.New(C.GoString(m.midi.msg))
	}
	return nil
...
func (m *midi) PortName(port int) (string, error) {
	...
	C.macmidi_get_port_name(m.midi, C.uint(port), nil, &bufLen)
	if !m.midi.ok {
		return "", errors.New(C.GoString(m.midi.msg))
	}
	...
	C.macmidi_get_port_name(m.midi, C.uint(port), p, &bufLen)
	if !m.midi.ok {
		return "", errors.New(C.GoString(m.midi.msg))
	}
...
func (m *midi) PortCount() (int, error) {
	n := C.macmidi_get_port_count(m.midi)
	if !m.midi.ok {
...
func (m *midi) Close() error {
	C.macmidi_close_port(C.MacMidiPtr(m.midi))
	if !m.midi.ok {
		return errors.New(C.GoString(m.midi.msg))
	}
	return nil
...
func NewMIDIInDefault() (MIDIIn, error) {
	in := C.macmidi_in_create_default()
	if !in.ok {
		defer C.macmidi_in_free(in)
		return nil, errors.New(C.GoString(in.msg))
	}
	return &midiIn{in: in, midi: midi{midi: C.MacMidiPtr(in)}}, nil
}
...
func NewMIDIIn(api API, name string, queueSize int) (MIDIIn, error) {
	p := C.CString(name)
	defer C.free(unsafe.Pointer(p))
	in := C.macmidi_in_create(C.enum_MacMidiApi(api), p, C.uint(queueSize))
	if !in.ok {
		defer C.macmidi_in_free(in)
		return nil, errors.New(C.GoString(in.msg))
	}
...
// Return the MIDI API specifier for the current instance of MacMidiIn.
func (m *midiIn) API() (API, error) {
	api := C.macmidi_in_get_current_api(m.in)
	if !m.in.ok {
...
func (m *midiIn) IgnoreTypes(midiSysex bool, midiTime bool, midiSense bool) error {
	C.macmidi_in_ignore_types(m.in, C._Bool(midiSysex), C._Bool(midiTime), C._Bool(midiSense))
	if !m.in.ok {
...
func (m *midiIn) CancelCallback() error {
	unregisterMIDIIn(m)
	C.macmidi_in_cancel_callback(m.in)
	if !m.in.ok {
		return errors.New(C.GoString(m.in.msg))
	}
	return nil
...
func (m *midiIn) Message() ([]byte, float64, error) {
	...
	sz := C.size_t(len(msg))
	r := C.macmidi_in_get_message(m.in, &msg[0], &sz)
	if !m.in.ok {
...
func NewMIDIOutDefault() (MIDIOut, error) {
	out := C.macmidi_out_create_default()
	if !out.ok {
		defer C.macmidi_out_free(out)
		return nil, errors.New(C.GoString(out.msg))
	}
	return &midiOut{out: out, midi: midi{midi: C.MacMidiPtr(out)}}, nil
}
...
func NewMIDIOut(api API, name string) (MIDIOut, error) {
	p := C.CString(name)
	defer C.free(unsafe.Pointer(p))
	out := C.macmidi_out_create(C.enum_MacMidiApi(api), p)
	if !out.ok {
		defer C.macmidi_out_free(out)
		return nil, errors.New(C.GoString(out.msg))
	}
...
// Return the MIDI API specifier for the current instance of MacMidiOut.
func (m *midiOut) API() (API, error) {
	api := C.macmidi_out_get_current_api(m.out)
	if !m.out.ok {
...
func (m *midiOut) SendMessage(b []byte) error {
	p := C.CBytes(b)
	defer C.free(unsafe.Pointer(p))
	C.macmidi_out_send_message(m.out, (*C.uchar)(p), C.int(len(b)))
	if !m.out.ok {
